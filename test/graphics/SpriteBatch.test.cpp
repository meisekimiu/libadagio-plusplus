...
public:
  explicit TestingSpriteBatch(MockGraphicsDevice *gd)
      : Adagio::SpriteBatch(gd) {
    gd->reset();
    renderingQueueCapacity = static_cast<int>(renderingQueue.capacity());
  }

  [[nodiscard]] int getCapacity() const { return renderingQueueCapacity; }
...
TEST_CASE("SpriteBatch reserves a specific size in rendering queue",
          "[SpriteBatch]") {
...
TEST_CASE("SpriteBatch renders things in order after calling end()",
          "[SpriteBatch]") {
  TestingSpriteBatch testSb(&gd);
  Adagio::SpriteBatch *sb = &testSb;
...
TEST_CASE("SpriteBatch preserves call order when z-indexes are equal",
          "[SpriteBatch]") {
  TestingSpriteBatch testSb(&gd);
  Adagio::SpriteBatch *sb = &testSb;
  Adagio::Texture2D texture(1, 1, 0, 0);
...
TEST_CASE("SpriteBatch pools TextState objects", "[SpriteBatch]") {
  ...
  sb->end();
  sb->begin();
  textPtr2 = sb->drawText("why do they call it oven when you of in the cold "
                          "food of out hot eat the food",
                          {0, 0});
  sb->end();
